...
# Use to format the string into a mat
...
for i in range(0, 17):
    for j in range(3, 17):
        if check_col(i, j) > max_value:
            max_value = check_col(i, j)
        if check_row(i, j) > max_value:
            max_value = check_row(i, j)
        if check_diag(i, j) > max_value:
            max_value = check_diag(i, j)
        if check_rdiag(i, j) > max_value:
            max_value = check_rdiag(i, j)
